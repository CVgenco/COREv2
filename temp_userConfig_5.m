...
config.jumpModeling.threshold = 0.1453;           % If 'std', this is the multiplier (e.g., 4 for 4x std). If 'percentile', this is the percentile (e.g., 95)
...
config.innovationDistribution.degreesOfFreedom = 10; % Leave empty to auto-calibrate to match historical kurtosis
...
config.innovationDistribution.degreesOfFreedom = 10; % Tune as needed (lower = fatter tails)
...
config.blockBootstrapping.blockSize = 73;  % Block size in time steps (e.g., 24 for 24-hour blocks)
...
config.regimeVolatilityModulation.amplificationFactor = 1.3200; % >1 amplify, <1 dampen, 1=no change
...
config.blockBootstrapping.blockSize = 73; % Block size (e.g., 24=1 day, 48=2 days, 168=1 week)
...
config.innovationDistribution.degreesOfFreedom = 10; % Only used if type is 't'

% Regime-Switching Volatility Modulation
config.regimeVolatilityModulation.enabled = true; % Set true to enable, false to disable
config.regimeVolatilityModulation.amplificationFactor = 1.3200; % Adjust as needed
